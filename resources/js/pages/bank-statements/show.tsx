...
import { AlertTriangle, ArrowDownCircle, ArrowLeft, ArrowUpCircle, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function BankStatementShow({ statement, lines, filters }: Props) {
            ...
            <div className="flex flex-1 flex-col gap-4 p-4">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-2xl font-semibold">Extras {statement.data_extras}</h1>
                            {statement.moneda && <Badge variant="secondary">{statement.moneda}</Badge>}
                            {statement.unallocated_count > 0 && (
                                <Badge variant="outline" className="border-amber-600/40 bg-amber-100/50 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300">
                                    ...
                                    {statement.unallocated_count} tranzacții nealocate
                                </Badge>
                            )}
                        </div>
                        <p className="text-sm text-muted-foreground">
                            {statement.banca ?? 'Bancă necunoscută'} · <span className="font-mono">{statement.iban}</span> · {statement.company.name}
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-xl font-semibold tabular-nums text-green-700 dark:text-green-400">
                                {formatAmount(statement.total_incoming, statement.moneda)}
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-xl font-semibold tabular-nums text-red-700 dark:text-red-400">
                                {formatAmount(statement.total_outgoing, statement.moneda)}
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            <div className={`text-xl font-semibold tabular-nums ${statement.total_unallocated > 0.01 ? 'text-amber-700 dark:text-amber-400' : ''}`}>
                                {statement.total_unallocated > 0.01
                                    ? formatAmount(statement.total_unallocated, statement.moneda)
                                    : '—'}
                            </div>
                        </CardContent>
                    </Card>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <Select
                        ...
                        onValueChange={(v) => applyFilter({ direction: v === 'all' ? null : v })}
                    >
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="incoming">Doar încasări</SelectItem>
                    ...
                    >
                        <AlertTriangle />
                        {filters.only_unallocated ? 'Doar nealocate' : 'Toate tranzacțiile'}
                    </Button>
                </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="w-24 text-right">Alocări</TableHead>
                                <TableHead className="w-36 text-right">Sumă</TableHead>
                                <TableHead className="w-36 text-right">Alocat</TableHead>
                                <TableHead className="w-36 text-right">Nealocat</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {lines.length === 0 && (
                                ...
                                return (
                                    <Fragment key={line.id}>
                                        <TableRow className={rowClass}>
                                            <TableCell className="p-0 text-center">
                                                {hasAllocations && (
                                                    <button
                                                        ...
                                                        aria-label={isExpanded ? 'Ascunde alocările' : 'Arată alocările'}
                                                    >
                                                        {isExpanded ? (
                                            ...
                                            <TableCell>{line.data_doc}</TableCell>
                                            <TableCell>
                                                <Badge variant="secondary">{line.tip_doc}</Badge>
                                            </TableCell>
                                            <TableCell className="font-medium">
                                                ...
                                                {(line.emitent || line.cine_preda || line.cine_primeste) && (
                                                    <div className="mt-0.5 text-[10px] font-normal uppercase tracking-wide text-muted-foreground/70">
                                                        {line.emitent && <span>op: {line.emitent}</span>}
                                                        {line.cine_preda && (
                                                            <span className="ml-2">predă: {line.cine_preda}</span>
                                                        )}
                                                        {line.cine_primeste && (
                                                            <span className="ml-2">primește: {line.cine_primeste}</span>
                                                        )}
                                                    </div>
                                                )}
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right tabular-nums font-medium">
                                                {formatAmount(line.val_mon, line.moneda)}
                                            </TableCell>
                                            <TableCell className="text-right tabular-nums text-muted-foreground">
                                                {formatAmount(line.val_allocated, line.moneda)}
                                            </TableCell>
                                            <TableCell className="text-right tabular-nums">
                                                {line.unallocated > 0.01 ? (
                                                    <span className="font-semibold text-amber-700 dark:text-amber-400">
                                                        {formatAmount(line.unallocated, line.moneda)}
                                                    </span>
                                                ) : (
                                                    '—'
                                                )}
                                                ...
                                                <TableCell colSpan={10} className="bg-muted/30 p-0">
                                                    <div className="p-3">
                                                        <div className="mb-2 text-xs font-semibold uppercase text-muted-foreground">
                                                            Facturi pe care s-a alocat ({line.allocations.length})
                                                        </div>
                                                        <Table>
                                                            <TableHeader>
                                                                <TableRow>
                                                                    <TableHead className="w-28">Dată factură</TableHead>
                                                                    <TableHead className="w-24">Tip</TableHead>
                                                                    <TableHead>Număr factură</TableHead>
                                                                    <TableHead>Partener</TableHead>
                                                                    <TableHead className="w-32">Stare</TableHead>
                                                                    <TableHead className="w-36 text-right">Alocat (monedă fact.)</TableHead>
                                                                    <TableHead className="w-36 text-right">Alocat (monedă plată)</TableHead>
                                                                    <TableHead className="w-36 text-right">Total factură</TableHead>
                                                                </TableRow>
                                                            </TableHeader>
                                                            <TableBody>
                                                                {line.allocations.map((alloc) => {
                                                                    ...
                                                                    return (
                                                                        <TableRow key={alloc.id}>
                                                                            <TableCell>{alloc.data_doc_com}</TableCell>
                                                                            <TableCell>
                                                                            ...
                                                                            <TableCell className="text-right tabular-nums">
                                                                                {formatAmount(alloc.val_com, inv?.moneda ?? null)}
                                                                            </TableCell>
                                                                            <TableCell className="text-right tabular-nums">
                                                                                {formatAmount(alloc.val_fin, line.moneda)}
                                                                            </TableCell>
                                                                            <TableCell className="text-right tabular-nums text-muted-foreground">
                                                                                {inv
                                                                                    ? formatAmount(
                                                                                          inv.val_mon + inv.val_mon_tva,
                                                                                          inv.moneda,
                                                                                      )
                                                                                    : '—'}
                                                                            </TableCell>
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: statement.data_extras, href: bankStatementsShow(statement.id) },
            ]}
        >
            {children}
        </AppLayout>
...
BankStatementShow.layout = (page: React.ReactNode) => <BankStatementShowLayout>{page}</BankStatementShowLayout>;
