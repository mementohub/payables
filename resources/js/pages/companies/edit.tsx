...
import { edit as companiesEdit, index as companiesIndex } from '@/routes/companies';
    ...
    return (
        <>
            ...
            <div className="flex flex-1 flex-col gap-4 p-4">
                ...
                <h1 className="text-2xl font-semibold">Modifică {company.name}</h1>
                        ...
                        <>
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Input id="cui" name="cui" defaultValue={company.cui ?? ''} />
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                <div className="grid gap-2">
                                    ...
                                    <Input id="db_host" name="db_host" defaultValue={company.db_host} required />
                                    <InputError message={errors.db_host} />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="db_port">Port BD</Label>
                                    <Input id="db_port" name="db_port" defaultValue={company.db_port} required />
                                    <InputError message={errors.db_port} />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="db_database">Bază de date</Label>
                                    <Input id="db_database" name="db_database" defaultValue={company.db_database} required />
                                    <InputError message={errors.db_database} />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="db_driver">Driver</Label>
                                    <Input id="db_driver" name="db_driver" defaultValue={company.db_driver} readOnly />
                                    <InputError message={errors.db_driver} />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="db_username">Utilizator BD</Label>
                                    <Input id="db_username" name="db_username" defaultValue={company.db_username} required />
                                    <InputError message={errors.db_username} />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="db_password">Parolă BD</Label>
                                    <Input id="db_password" name="db_password" type="password" autoComplete="new-password" placeholder="•••••• (nemodificată)" />
                                    <InputError message={errors.db_password} />
                                </div>
                            </div>

                            <Button disabled={processing}>Salvează modificările</Button>
                        </>
                    )}
                </Form>
            </div>
